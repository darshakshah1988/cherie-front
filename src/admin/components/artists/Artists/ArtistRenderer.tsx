...
function renderArtistGrid(artists: Array<ArtistData>) {
  if (Array.isArray(artists) && artists.length > 0) {
    return (
      <Fragment>
        {artists.map(item => {
          return (
            <ArtistRenderer.GridCol key={item.id}>
              <ArtistGridCard {...item} />
...
function renderArtistList(artists: Array<ArtistData>) {
  if (Array.isArray(artists) && artists.length > 0) {
    return (
      <Fragment>
        {artists.map(item => {
          return (
            <ArtistRenderer.ListCol key={item.id}>
              <ArtistListCard {...item} />
